...
The `ENCRYPTION_KEY` and `JWT_SECRET` are managed as **Cloudflare Secrets** in production and stored in `.dev.vars` for local development. They are auto-generated by `npm run setup-cf` and are never stored in the database.
...
cp .dev.vars.example .dev.vars  # Create local secrets file
npm run setup-cf                # Auto-generates ENCRYPTION_KEY + JWT_SECRET into .dev.vars
npm run db:migrate:local        # Create local D1 tables
npm run dev                     # Start wrangler dev server → http://localhost:8787
```

- **`.dev.vars` is required** — it must contain `ENCRYPTION_KEY` and `JWT_SECRET` (see `.dev.vars.example`)
- `npm run setup-cf` auto-generates these keys if they are missing from `.dev.vars`
- First visit → Setup wizard (create admin account)
