...
| Turnstile (bot protection) | Auto-provisioned by `setup-cf` as Cloudflare Secrets |
...
2. **Register a workers.dev subdomain** _(one-time, skip if you've deployed a Worker before)_. In the Cloudflare Dashboard, go to **Workers & Pages** in the left sidebar, then **Overview**. If you haven't deployed a Worker before, you'll be prompted to claim your free `<your-subdomain>.workers.dev` subdomain. Your app will be reachable at `<INSTANCE_NAME>.<your-subdomain>.workers.dev`.
3. Go to your Cloudflare Dashboard -> My Profile -> API Tokens -> Create Token -> Create Custom Token.
...
5. Add the following **Repository Variable** (not a secret):
    - `INSTANCE_NAME`: The prefix for your app (e.g., `my-e-zer0`).
6. Go to the **Actions** tab in GitHub and run the `Deploy e-zer0` workflow.
...
e-zer0 protects the login and setup pages with [Cloudflare Turnstile](https://www.cloudflare.com/products/turnstile/). MCP routes are unaffected — they authenticate via JWT bearer tokens.
