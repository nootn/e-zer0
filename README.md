...
| What              | How                                                  |
| ----------------- | ---------------------------------------------------- |
| Encryption keys   | Auto-generated on first run, stored in D1            |
| JWT secrets       | Auto-generated on first run, stored in D1            |
...
**No `.env` files. No `.dev.vars`. No `wrangler secret put`.** Just deploy and open the dashboard.
...
2. Go to your Cloudflare Dashboard -> My Profile -> API Tokens and create a token with **Edit** permissions for Workers, D1, and Vectorize. Find your **Account ID** in the Cloudflare URL or dashboard sidebar.
...
That's it — just 2 secrets (both Cloudflare). The GitHub Action will automatically provision your database, set up your vector index, and deploy the web UI. Once deployed, open the dashboard, create your admin account (encryption keys are auto-generated), then configure your OAuth providers through the **Settings** page.
...
No `.dev.vars` or secrets needed — the setup wizard generates everything on first launch.
