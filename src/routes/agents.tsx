...
const agents = new Hono<{ Bindings: Env; Variables: { userId: number; username: string } }>();

agents.get('/', async (c) => {
    const username = c.get('username');
    const message = c.req.query('message');
    const error = c.req.query('error');
    // Check for newly created credentials in query params (one-time display)
    const newClientId = c.req.query('new_client_id');
...
agents.post('/create', async (c) => {
    ...
    return c.redirect(
        `/agents?new_client_id=${encodeURIComponent(clientId)}&new_secret=${encodeURIComponent(clientSecret)}&new_name=${encodeURIComponent(name)}`
    );
});
